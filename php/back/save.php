...
try {
    if ($type === 'artiste') {
        ...
        if ($id) {
            $stmt = $db->prepare("UPDATE artistes SET nom = :nom, id_genre = :id_genre, location = :location, description = :description, photo = :photo, photo2 = :photo2, reseaux_sociaux = :reseaux_sociaux, audio_url = :audio_url, decouverte = :decouverte, cdc = :cdc WHERE id_artiste = :id");
            ...
            $stmt = $db->prepare("INSERT INTO artistes (nom, id_genre, location, description, photo, photo2, reseaux_sociaux, audio_url, decouverte, cdc) VALUES (:nom, :id_genre, :location, :description, :photo, :photo2, :reseaux_sociaux, :audio_url, :decouverte, :cdc)");
            $stmt->bindParam(':nom', $nom, PDO::PARAM_STR);
            $stmt->bindParam(':id_genre', $id_genre, PDO::PARAM_INT);
            $stmt->bindParam(':location', $location, PDO::PARAM_STR);
            $stmt->bindParam(':description', $description, PDO::PARAM_STR);
